...
The `Stream Deck Plugin Template` is a template to let you get started quickly when writing a JavaScript plugin
for [Stream Deck](https://developer.elgato.com/documentation/stream-deck/). `Stream Deck Plugin Template` requires
Stream Deck 6.0 or later.
...
`com.gabe565.stopwatch` with `my.domain.plugin-name`
...
```javascript
const myAction = new Action("com.gabe565.stopwatch.action");
